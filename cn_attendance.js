...
window.cnRenderGrid=function(){
  ...
  $('cn-grid-body').querySelectorAll('.cn-cell').forEach(inp=>{
    inp.addEventListener('change',e=>{
      ...
      RECS[ri].days[di]=e.target.value.trim();
      persist();
      const td=e.target.parentElement;
      td.className=cellCls(parse(e.target.value));
    });
  });
...
window.cnRenderDetail=function(){
  ...
  $('cn-detail-body').innerHTML=rows.length?rows.map(r=>`<tr>
    ...
    <td class="num hl-ot">${r.cnY||''}</td><td class="num hl-ot">${r.cnW||''}</td><td class="num hl-ot">${r.owY||''}</td><td class="num hl-ot">${r.owW||''}</td>
    <td class="num">${r.region}</td><td class="num">${r.zhuwai}</td><td class="num">${r.actual==null?'—':r.actual}</td><td class="num">${r.meal}</td>
  </tr>`).join(''):'<tr><td colspan="21" style="text-align:center;padding:30px;color:var(--color-text-tertiary)">无符合条件的记录</td></tr>';
...
window.cnExportAnalysis=function(){
  ...
  const head=['工号','姓名','部门','科室','职类','总出勤','驻外出勤','国内出勤','探亲假','事假','病假','带薪假','法定节假日','国内延时(h)','国内周末(h)','国外延时(h)','国外周末(h)','地区补贴','驻外补贴','实际海外出勤','餐补'];
  const aoa=[head,...rows.map(r=>[r.id,r.name,r.dept,r.section,r.group,r.total,r.owDays,r.cnDays,r.qj,r.sj,r.bing,r.paid,r.hl,r.cnY,r.cnW,r.owY,r.owW,r.region,r.zhuwai,r.actual==null?'':r.actual,r.meal])];
  ...
  r.onload=function(ev){
    try{
      ...
      for(let ri=1;ri<rows.length;ri++){
        ...
        for(let d=1;d<=DIM;d++){if(dayCol[d]!=null){const v=row[dayCol[d]];rec.days[d-1]=(v==null?'':String(v)).trim();}}
        matched++;
      }
      persist();
      const ana=$('cn-analysis-panel');
